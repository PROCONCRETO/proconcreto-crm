...
## Los modales ya no se cierran al hacer clic afuera (2026-08-25)

A pedido del usuario ("se pincha afuera con el mouse y se cierra automáticamente la ventana y se pierde el trabajo adelantado") — se quitó, en `js/historico-clientes-stats.js`, un listener genérico que estaba puesto sobre TODOS los `.modal-overlay` de la app (clic en el fondo oscuro → `classList.remove('abierto')`, sin pasar por `cerrarModal()`). Era el único mecanismo de cierre por clic-afuera en todo el código (no había nada por Escape ni por-modal aparte). Ahora una ventana solo se cierra con su propio botón de Guardar/Cancelar/✕ — comportamiento parejo para las 25 pantallas `.modal-overlay` del aplicativo (creación de cliente, Diseño de Mezcla, Costeo de Producto, etc.), sin tener que tocarlas una por una.
...
### Pantalla de Inicio (2026-08-27)

A pedido del usuario: "creemos una pantalla de inicio, cuyo botón de acceso sea el logo de ProSuite... botones medianos-grandes de cada una de las ventanas principales". `pantalla-inicio` es ahora la `.pantalla` activa por defecto en el HTML estático (antes lo era `pantalla-nueva-cotizacion`) — lo primero que se ve siempre al abrir el programa, un botón mediano-grande (`.inicio-modulo`) por cada uno de los 5 módulos de la barra superior (Cotizaciones y Ventas, Producción, Logística, Calidad, Centro de Costos — este último oculto para quien no tiene acceso, mismo criterio que ya ocultaba el botón del nav superior, ver `_esUsuarioCentroCostos()`). Cada botón llama `activarModulo(modulo)`, igual que el botón correspondiente de la barra superior.

- **El logo de ProSuite (`.nav-logo`) es el botón de acceso a Inicio** — ganó `onclick="irInicio()"` (antes era un `<div>` decorativo sin interacción) y `cursor:pointer`. `irInicio()` (`js/navegacion.js`) apaga todas las `.pantalla`/`.nav-modulo.activo`, oculta los 5 subnav (ninguno aplica en Inicio) y activa `pantalla-inicio`.
- **`activarModulo()` dejó de depender de `event.currentTarget`** para resaltar el botón del módulo activo en la barra superior — antes asumía que quien disparaba la función siempre era el propio botón `.nav-modulo` (cierto mientras la única forma de cambiar de módulo era la barra superior), pero un botón de Inicio también llama `activarModulo()` y no es un `.nav-modulo`. Ahora busca el botón correcto por el parámetro `modulo` (`document.querySelector(\`.nav-modulo[onclick*="'${modulo}'"]\`)`), así que el resaltado queda correcto sin importar desde dónde se llamó.
- `.inicio-grid` usa `flex-wrap` centrado, no CSS grid de columnas fijas — con 5 módulos, un grid deja el último renglón pegado a la izquierda (4 arriba + 1 suelto); centrado se ve balanceado en cualquier ancho de pantalla.
- Verificado con clics reales (no solo revisión de código) sobre `cotizaciones.html` real: estado inicial con Inicio activo y ningún módulo resaltado en la barra; clic en el botón "Producción" de Inicio activa la pantalla de Producción Y resalta el módulo correcto en la barra superior (la parte que se rompía sin el arreglo de `event.currentTarget`); clic en el logo vuelve a Inicio sin ningún módulo resaltado.
