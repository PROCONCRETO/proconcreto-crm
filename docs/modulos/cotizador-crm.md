...
**Campo Dirección (2026-08-26)**: antes cada proyecto era una fila de tabla de 3 columnas (Proyecto/Contacto/Teléfono) — se pasó a tarjeta (una por proyecto, con `.form-grid`/`.form-grupo`, mismo estilo que las tarjetas de entrega/diligencia de Logística) para poder agregar Dirección sin apretar el modal (560px de ancho). El campo Nombre del proyecto es una etiqueta corta pensada para identificarlo en la app ("Torres del Parque"), no necesariamente algo que Google Maps reconozca por sí solo — Dirección es un texto aparte, pensado específicamente para alimentar el link de Google Maps del imprimible de Programación de Viajes en Logística (ver `docs/modulos/logistica.md`). Es opcional: si no se llena, el link de Maps sigue funcionando con el nombre del proyecto + la ciudad del viaje, solo que con menos precisión. Al elegir el proyecto en una entrega (`_alElegirProyectoEntrega()` en `js/logistica.js`), la dirección se copia a `entrega.direccionEntrega` — un campo interno que no se muestra en pantalla, solo alimenta el link.
