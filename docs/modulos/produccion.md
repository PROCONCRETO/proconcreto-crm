...
- **Tarjetas KPI** (`#est-prod-tarjetas`): Unidades de primera producidas, Merma (ud), Segundas (ud), % Deficiencia (semáforo, mismos umbrales ya usados en Producción Diaria: verde ≤5%, ámbar ≤10%, rojo por encima — `_colorDeficiencia()`), Cemento / unidad (primera) — mismas fórmulas ya establecidas en la sección "Merma y Segundas" de arriba, agregadas sobre el período filtrado.
- **Tendencia de producción** (`_chartTendenciaProduccion()`): línea de una sola serie (Primera, verde `#0ca30c`) por día, omitiendo los días sin producción de primera — mismo mecanismo que `_chartTendencia()` de Logística.
- **Tendencia de merma y segundas** (`_chartTendenciaDeficiencia()`) — **gráfica aparte, con su propio eje** (2026-09-09, a pedido del usuario): al principio Merma/Segundas compartían eje con Primera en una sola gráfica de 3 series, pero al ser números mucho más chicos quedaban pegadas al piso, casi ilegibles. Separarlas en su propia gráfica (rojo `#d03b3b` merma, ámbar `#fab219` segundas — misma paleta de estado fija que ya usa `estadisticas-logistica.js`) deja ver su tendencia real. Usa un criterio de "día con actividad" propio y distinto al de la gráfica de Primera — omite los días sin NINGUNA merma/segundas, aunque ese día sí haya habido producción de primera (no tendría sentido ensuciar esta gráfica con ceros de días donde simplemente no hubo ningún problema).
- **Ranking de productos por volumen** (`_chartRankingVolumen()`): top 10 por unidades de primera, barra horizontal azul — mismo patrón que "Destinos más frecuentes" de Logística.
...
- Verificado con un arnés real — 21 aserciones (tarjetas KPI excluyen correctamente pretensados y productos sin Costeo, la nota de "sin clasificar" aparece con el conteo correcto, la gráfica de Primera y la de Merma/Segundas suman los totales esperados por separado, los rankings ordenan y colorean bien, el umbral de 5 unidades excluye productos de bajo volumen del ranking de deficiencia sin excluirlos de las tarjetas, y el cambio de período recalcula todo) — más capturas de pantalla reales revisando el layout — sin errores de consola.
