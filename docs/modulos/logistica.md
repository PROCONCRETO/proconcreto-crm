...
- **Mover un viaje a otro día**: arrastrando el chip del calendario y soltándolo sobre otro día (drag & drop nativo, sin librerías — `iniciarArrastreViaje()`/`soltarViajeEnDia()`), incluso a días futuros. Solo se puede arrastrar un viaje en fecha no bloqueada, y no se puede soltar sobre una fecha bloqueada (mismo criterio que `esFechaBloqueada`) — un viaje que ya pasó se reprograma desde "✅ Cumplidos", no arrastrándolo.
  - Si el viaje arrastrado es de **hoy** (ya cuenta en las estadísticas "hasta hoy"), no se le cambia la fecha en el sitio — eso lo haría desaparecer de esas estadísticas en vez de contar como reprogramado. En su lugar se deja el mismo rastro que "Reprogramar" en Cumplidos: las entregas pendientes del viaje original quedan marcadas `reprogramada` (el viaje se queda hoy, con ese resultado) y se crea un viaje nuevo en la fecha destino con copias frescas (`pendiente`) de esas entregas. Las entregas que ya tenían un resultado real (Hecha/Cancelada) no se tocan.
  - Si el viaje arrastrado es de un día **futuro** moviéndose a otro día futuro, simplemente se le cambia la fecha en el sitio (no entra en ninguna ventana de estadísticas "hasta hoy" ni antes ni después, no hay nada que preservar).
- **`entrega.fechaOriginal`**: se fija la primera vez que la entrega se guarda y nunca vuelve a cambiar — queda como rastro/auditoría de cuándo se programó originalmente, aunque la entrega termine en otro día.
...
`renderEstadisticasLogistica()`, filtrable por periodo (7/30/90 días o todo, siempre acotado a "hasta hoy" — el futuro no tiene cumplimiento que medir todavía). "Viajes en el periodo" y "Entregas programadas" cuentan todo lo programado; **"Peso transportado", "% Capacidad promedio" y "Desempeño por vehículo" solo cuentan entregas marcadas "Hecha"** (agrupadas por viaje real — un viaje con 2 entregas hechas es 1 viaje, no 2), para no mezclar lo programado con lo realmente cumplido. **"Entregas reprogramadas"** cuenta `cumplido.estado === 'reprogramada'` (mismo bucket que la dona) — tanto "Reprogramar" en Cumplidos como arrastrar un viaje de hoy a otro día dejan ese rastro; un cambio dentro del mismo día nunca se marca así, por diseño. Gráficas:
