...
- Se agregó un campo nuevo por cada material — `cementoProducto`, `metacaolinProducto`, `arenaProducto`, `gravaProducto`, `aguaProducto` — que guarda el `nombre` del ítem real en `insumos_costos` (Costos de Referencia) que le da precio a ese material. En el modal de Nuevo/Editar Diseño, la sección "MATERIALES POR M³" se rediseñó como tabla, con un `<select>` de "Producto" (poblado en vivo con `_opcionesProductoCatalogo()` desde `INSUMOS_COSTOS`, filtrado a `categoria` `materia_prima`/`insumo_cif`) al lado de cada cantidad.
- Cada aditivo (`{ tipo, producto, dosis }`) ahora también tiene `producto` — **`tipo`** (Superplastificante/Retardante/Acelerante) es la categoría funcional que sigue usando Ajuste Diario para sumar dosis (`sumaPorTipo()`), no se puede volver un producto libre sin romper ese cálculo; **`producto`** es aparte, el ítem específico de Costos de Referencia (ej. "Sika Viscocrete 20") que le da precio a esa dosis.
- **Regla dura al guardar** (`guardarDiseno()`): si un material o un aditivo tiene una cantidad/dosis mayor a 0, su "Producto" es obligatorio — si el producto que hace falta todavía no existe en Costos de Referencia, el guardado se bloquea con un mensaje que dice exactamente cuál falta y dónde crearlo. Esto aplica tanto a diseños nuevos como a ediciones de uno existente (no hay excepción para diseños viejos — al día de este cambio no había ningún diseño guardado todavía, así que no hubo migración que hacer).
- **Catálogo sembrado el 2026-07-30** para que "muestra las materias primas de los diseños de mezcla en Costos de Referencia" tuviera algo que mostrar de entrada: `Arena` (materia_prima, kg), `Triturado Grueso` (materia_prima, kg), `Metacaolín` (materia_prima, kg), `Agua` (insumo_cif, L) — los 4 con `valorUnitario: 0` como placeholder, pendientes de que el usuario les ponga precio real. **`Cemento` no se sembró** porque el usuario ya había creado a mano "Cemento estructural granel" con precio real ($540/kg) — sembrar un "Cemento" genérico habría creado un duplicado confuso; el picker de Cemento en Diseño de Mezcla simplemente arranca sin selección hasta que el usuario elija cuál de los productos de cemento del catálogo aplica.
