...
- `diseno.materiales` sigue teniendo las mismas llaves fijas de siempre (`cemento`, `metacaolin`, `arena`, `grava`, `agua`, `absorcionArena`, `absorcionTriturado`, `aditivos`) — **no se tocaron**, porque Ajuste Diario de Mezcla (`js/calidad-ajuste-mezcla.js:381-392`) lee exactamente esas llaves para el cálculo de humedad y no debía romperse.
- Se agregó un campo nuevo por cada material — `cementoProducto`, `metacaolinProducto`, `arenaProducto`, `gravaProducto`, `aguaProducto` — que guarda el `nombre` del ítem real en `insumos_costos` (Costos de Referencia) que le da precio a ese material. En el modal de Nuevo/Editar Diseño, la sección "MATERIALES POR M³" se rediseñó como tabla, con un `<select>` de "Producto" al lado de cada cantidad.
- **El picker de "Producto" está filtrado por rol, no muestra el catálogo completo** (corregido 2026-07-30 tras detectar que la primera versión mostraba TODOS los ítems de `insumos_costos` sin relación con la fila — ej. el picker de "Cemento" también ofrecía "Agua" o "Arena"). Cada ítem de Costos de Referencia tiene ahora un campo opcional `rolDiseno` (`'cemento'|'metacaolin'|'arena'|'grava'|'agua'|'aditivo'|''`), elegido en el modal Nuevo/Editar Ítem de esa pantalla ("Rol en Diseño de Mezcla"). `_opcionesProductoCatalogo(seleccionado, rol)` en `js/calidad-mezclas.js` filtra `INSUMOS_COSTOS` por `i.rolDiseno === rol` — así el picker de la fila "Cemento" solo ofrece ítems marcados con rol `cemento`, etc. Un ítem sin rol asignado (la mayoría de Insumos/CIF que no tienen nada que ver con mezcla, ej. "Ensayo de absorción") sigue viviendo normal en Costos de Referencia pero nunca aparece en ningún picker de Diseño de Mezcla.
- Cada aditivo (`{ tipo, producto, dosis }`) ahora también tiene `producto` (filtrado por rol `'aditivo'`) — **`tipo`** (Superplastificante/Retardante/Acelerante) es la categoría funcional que sigue usando Ajuste Diario para sumar dosis (`sumaPorTipo()`), no se puede volver un producto libre sin romper ese cálculo; **`producto`** es aparte, el ítem específico de Costos de Referencia (ej. "Sika Viscocrete 20") que le da precio a esa dosis.
- **Regla dura al guardar** (`guardarDiseno()`): si un material o un aditivo tiene una cantidad/dosis mayor a 0, su "Producto" es obligatorio — si el producto que hace falta todavía no existe en Costos de Referencia (o existe pero no tiene el rol correcto asignado, así que no sale en el picker), el guardado se bloquea con un mensaje que dice exactamente cuál falta y dónde crearlo. Esto aplica tanto a diseños nuevos como a ediciones de uno existente (no hay excepción para diseños viejos — al día de este cambio no había ningún diseño guardado todavía, así que no hubo migración que hacer).
- **Catálogo sembrado el 2026-07-30** para que "muestra las materias primas de los diseños de mezcla en Costos de Referencia" tuviera algo que mostrar de entrada, ya con su rol asignado: `Arena` (rol `arena`), `Triturado Grueso` (rol `grava`), `Metacaolín` (rol `metacaolin`), `Agua` (rol `agua`) — los 4 con `valorUnitario: 0` como placeholder, pendientes de que el usuario les ponga precio real. **`Cemento` no se sembró** porque el usuario ya había creado a mano "Cemento estructural granel" con precio real ($540/kg) — se le agregó `rolDiseno: 'cemento'` a ese ítem existente en vez de crear uno genérico duplicado.
