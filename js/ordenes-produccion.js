...
function renderOrdenes() {
  ...
  tbody.innerHTML = data.map(o => `
    <tr style="border-top:2px solid var(--azul-oscuro)">
      ...
      <td>
        ${o.cotizacion ? `<span style="font-weight:600;color:var(--azul)">${_numeroCotTexto(COTIZACIONES.find(c => c.numero === o.cotizacion) || { numero: o.cotizacion })}</span> <span style="font-size:11px;background:var(--gris-borde);color:var(--gris-texto);padding:2px 6px;border-radius:3px;font-weight:600">${_esc(o.versionCotizacion)}</span>` : '—'}
      </td>
      <td style="font-weight:600">${_esc(o.cliente)}</td>
      <td style="color:var(--gris-medio);max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="${_esc(o.descripcion)}">${_esc(o.descripcion)}</td>
...
function verPDFOrden(id) {
  ...
  const filasItems = items.map(it => {
    ...
    return `<tr>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;color:#555;font-size:11px">${_esc(it.codigo)}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:12px">${_esc(it.nombre)}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:center;font-size:12px">${it.cantidad}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:12px">${_esc(it.unidad)}</td>
  ...
  <div id="os-preview-doc" style="font-family:Arial,sans-serif;max-width:780px;margin:0 auto;padding:0;color:#222">
    ...
    <div style="background:#001F3F;color:white;padding:18px 24px;display:flex;align-items:center;justify-content:space-between">
      ...
      <div style="background:#1D9E75;padding:8px 16px;border-radius:6px;text-align:right">
        <div style="font-size:20px;font-weight:700">${_esc(o.numero)}</div>
    ...
    <div style="background:#E8F5E9;border-left:4px solid #1D9E75;padding:8px 16px;font-size:12px;color:#1B5E20">
      Generada desde cotización <strong>${o.cotizacion ? _numeroCotTexto(COTIZACIONES.find(c => c.numero === o.cotizacion) || { numero: o.cotizacion }) : '—'} ${_esc(o.versionCotizacion)}</strong> — Estado: Aceptada
    </div>
    <!-- Cliente -->
    <div style="padding:14px 16px;border-bottom:1px solid #eee">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
        <div><div style="font-size:10px;color:#888">Razón social</div><div style="font-size:13px;font-weight:600;margin-top:2px">${_esc(o.clienteData?.nombre || o.cliente)}</div></div>
        <div><div style="font-size:10px;color:#888">Contacto</div><div style="font-size:12px;margin-top:2px">${_esc(o.clienteData?.contacto) || '—'}</div></div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid #eee">
      <div style="padding:14px 16px;border-right:1px solid #eee">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <div><div style="font-size:10px;color:#888">Destino</div><div style="font-size:12px;margin-top:2px">${_esc(o.transporte?.destino) || '—'}</div></div>
          <div><div style="font-size:10px;color:#888">Cargue</div><div style="font-size:12px;margin-top:2px;text-transform:capitalize">${_esc(o.cargue) || 'No'}</div></div>
        </div>
      </div>
      <div style="padding:14px 16px">
        <div style="font-size:10px;font-weight:700;color:#888;letter-spacing:0.08em;margin-bottom:8px">CONDICIONES</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <div><div style="font-size:10px;color:#888">Forma de pago</div><div style="font-size:12px;margin-top:2px">${_esc(o.condiciones?.pago) || '—'}</div></div>
          <div><div style="font-size:10px;color:#888">Tiempo entrega</div><div style="font-size:12px;margin-top:2px">${_esc(o.condiciones?.entrega) || '—'}</div></div>
          ...
          <div><div style="font-size:10px;color:#888">Prioridad</div><div style="font-size:12px;margin-top:2px">${_esc(o.prioridad) || 'Normal'}</div></div>
        </div>
      </div>
    </div>
    <!-- Observaciones -->
    <div style="padding:12px 16px;border-bottom:1px solid #eee">
      ...
      <div style="font-size:12px;color:#555;min-height:28px">${_esc(o.observaciones) || '—'}</div>
    </div>
    <!-- Firmas -->
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0;border-bottom:1px solid #eee">
      ${[
        { label: 'Aprobado por', persona: _esc(o.vendedor?.nombre) || '—', cargo: _esc(o.vendedor?.cargo) || '' },
  ...
  document.getElementById('contenido-preview').innerHTML = `
    <div class="no-print" style="background:#1C2333;color:white;padding:12px 24px;display:flex;align-items:center;gap:16px">
      <span style="font-weight:700">Orden de Producción — ${_esc(o.numero)}</span>
